...
async fn main() -> Result<(), sqlx::Error> {
    ...
    insert_medical_report_providers(&pool).await;
...
async fn insert_medical_report_providers(db_pool: &Pool<MySql>) -> () {
    let data = MedicalReportProvider {
        provider_id: None,
        provider_name: Some(String::from("Test provider in rust")),
    };

    let result = sqlx::query!("INSERT INTO medical_report_provider (provider_name) VALUES
                (?);", data.provider_name)
        .execute(db_pool)
        .await;
...
// async fn reset_db(db_pool: &Pool<MySql>) -> Result<MySqlQueryResult, Error> {
//     let result = sqlx::query(r#"DROP DATABASE IF EXISTS medical_db;
