...
CREATE TABLE medical_report (
    ...
    report_provider     INT             NOT NULL        DEFAULT '1',
    report_type         INT             NOT NULL        DEFAULT '1',
    PRIMARY KEY (report_id),
    CONSTRAINT FOREIGN KEY (report_provider) REFERENCES medical_report_provider (provider_id) ON DELETE SET DEFAULT,
    CONSTRAINT FOREIGN KEY (report_type) REFERENCES medical_report_type (report_type_id) ON DELETE SET DEFAULT
);
...
CREATE TABLE provider_physical_address (
    ...
    CONSTRAINT FOREIGN KEY (provider_id) REFERENCES medical_report_provider (provider_id) ON DELETE SET DEFAULT
);
...
CREATE TABLE provider_contact_name (
    ...
    provider_contact_classification  INT             NOT NULL        DEFAULT '1',
    provider_contact_last_name       VARCHAR(50)     NOT NULL,
    provider_contact_first_name      VARCHAR(50),
    PRIMARY KEY (provider_contact_id),
    CONSTRAINT FOREIGN KEY (provider_id) REFERENCES medical_report_provider (provider_id) ON DELETE SET DEFAULT,
    CONSTRAINT FOREIGN KEY (provider_contact_classification) REFERENCES provider_contact_position_classification (classification_id) ON DELETE SET DEFAULT
);
...
CREATE TABLE medical_report_value_units (
...
CREATE TABLE medical_report_value_type (
...
CREATE TABLE medical_report_data (
    ...
    CONSTRAINT FOREIGN KEY (value_type) REFERENCES medical_report_value_type (value_type_id) ON UPDATE CASCADE,
    CONSTRAINT FOREIGN KEY (value_unit) REFERENCES medical_report_value_units (value_unit_id) ON UPDATE CASCADE
);
...
INSERT INTO medical_report_value_units (value_unit_full_name, value_unit_description, value_unit_shorthand) VALUES
...
INSERT INTO medical_report_value_type (value_type_name, value_type_abbreviation) VALUES
    ("Albumin", "ALB"),
    ("Carbon Dioxide", "CO2"),
...
-- INSERT INTO medical_report_data (report_id, value_type, value_unit, value_measurement) VALUES
--     (1, 1, 1, 3.37);
...
-- FROM 
-- 	medical_report_data mrd
--   	JOIN medical_report_value_units mrvu
--   		ON mrvu.value_unit_id = mrd.value_type
--    	JOIN medical_report_value_type mrvt
